	 ...
	 class fkEditor {
		 constructor(id) {
			 ...
			 this.config.menu.forEach((v) => {
				 if(v.fk == 'uploadImg'){
					 fkMenu.innerHTML += '<label data-fk="' + v.fk + '"><input type="file" id="upload-img" accept="image/*" multiple="multiple"/>上传图片</label>';
				 }else if(v.fk == 'uploadFile'){
					 fkMenu.innerHTML += '<label data-fk="' + v.fk + '"><input type="file" id="upload-file" multiple="multiple"/>上传文件</label>';
				 }else{
		 ...
		 runFka(t) {
			 if (t == 'key') {
				 ...
				 var _this = this;
				 SX(t == 'uploadImg' ? '#upload-img' : '#upload-file')[0].onchange = function () {
					 var that = this;
					 var formData = new FormData();
					 for (var i = 0; i < this.files.length; i++) {
						 formData.append('file[]', this.files[i]);
					 }
					 SX.ajax(SX.CONF.URL + 'upload', formData).then(res => {
						 if (res.error) {
							 alert(res.data);
						 } else {
							 that.value = null;
