...
class fk {
	...
	public function getStyle($t){
		...
		foreach ($arr as $v) {
			...
			if(isset($this->mark[$v])) $style[]=$this->mark[$v];
		}
		$style = implode(' ', $style);
	...
	private function fkReg($str) {
		...
		if($a1 == '['){
			...
			if(preg_match('/^\[(.+?)\s+(.+?)\](.*)/',$text,$match)){
				...
				if($style) $text='<span class="'.$style.'">'.$match[2].'</span>'.$match[3];
			}
			elseif(preg_match('/^\[(.+?)\]/',$text,$match)){
				$style = $this->getStyle($match[1]);
				if($style){
					$text = trim(substr($text,strlen($match[1])+2));
					$text='<p class="'.$style.'">'.$text.'</p>';
				}
			}
		}
		//列表
		...
		$text=preg_replace_callback('/(?<!\\\)\[(.+?)\s+(.+?)(?<!\\\)\]/', function($m){
			...
			if($style) return '<span class="'.$style.'">'.$m[2].'</span>';
			return $m[0];
		}, $text);
		...
		$text=preg_replace('/(?<!\\\)\[img\s+([^\s\]]*)\s*([^\s\]]*)\s*([^\s\]]*)\](?<!\\\)/is','<img src="$1" alt="$2" title="$3"/>',$text);
