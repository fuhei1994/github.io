...
<div class="info">
	...
	<a href="javascript:checkUpdate()">检查更新</a>
	<a href="javascript:systemSync()">系统同步</a>
	<a href="javascript:dbSync()">数据库版本同步</a>
	<script>
		function dbSync(){
			SX.alert({
		...
		function checkUpdate(a){
			SX.ajax('{url admin/checkUpdate}').then(res=>{
				if(res.error){
					...
					if(res.data){
						SX.alert({
							content:`溯雪已有新版本<strong>${res.data}</strong>，更新之前，请做好数据备份哦，以免更新失败造成数据丢失！是否更新？`,
							btn:['拒绝','立即更新'],
							yes(){
								SX.pop('更新中，请稍等...',0);
								SX.ajax('{url admin/update}').then(res=>{
